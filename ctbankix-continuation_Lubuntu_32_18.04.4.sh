...
if ! head -1 /etc/issue | grep -q 'Ubuntu 18.04.4' || ! [ $(getconf LONG_BIT) == '32' ]
then
	echo "Sie benutzen die falsche (L)Ubuntu-Version. Bitte (L)Ubuntu 18.04.4 32 Bit verwenden."
	exit
fi
...
wget -c -N -P source http://cdimage.ubuntu.com/lubuntu/releases/18.04.4/release/lubuntu-18.04.4-desktop-i386.iso
wget -c -N -P source http://cdimage.ubuntu.com/lubuntu/releases/18.04.4/release/lubuntu-18.04.4-desktop-amd64.iso

mount -o loop source/lubuntu-18.04.4-desktop-amd64.iso /mnt/
mkdir iso
...
mount -o loop source/lubuntu-18.04.4-desktop-i386.iso /mnt/
...
menu title c't Bankix Lubuntu 18.04.4

label ctbankix
  menu label c't Bankix Lubuntu 18.04.4
...
menuentry "c't Bankix Lubuntu 18.04.4" {
	set gfxpayload=keep
