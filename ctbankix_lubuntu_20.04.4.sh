...
#	Copyright (C) 2022 ctbankix-continuation-team
#
...
KERNEL_CONFIG_FOLDER=debian.hwe-5.13/config
...
sed -i "/^CONFIG_NVME_/ s/'[mny]'/'-'/g" $KERNEL_CONFIG_ANNOTATIONS

sed -i "/^CONFIG_BLK_DEV_NVME/ s/'[my-]'/'n'/g" $KERNEL_CONFIG_ANNOTATIONS
sed -i "/^CONFIG_NVME_FC / s/'[my-]'/'n'/g" $KERNEL_CONFIG_ANNOTATIONS
sed -i "/^CONFIG_NVME_RDMA / s/'[my-]'/'n'/g" $KERNEL_CONFIG_ANNOTATIONS
sed -i "/^CONFIG_NVME_TARGET / s/'[my-]'/'n'/g" $KERNEL_CONFIG_ANNOTATIONS
sed -i "/^CONFIG_NVME_TCP / s/'[my-]'/'n'/g" $KERNEL_CONFIG_ANNOTATIONS
# <<<<< Quellcode anpassen <<<<<
...
wget -c -N -P source http://cdimage.ubuntu.com/lubuntu/releases/${UBUNTU_VERSION}/release/lubuntu-${UBUNTU_VERSION}-desktop-amd64.iso

# Mounten und Verzeichnisse kopieren
mount -o loop source/lubuntu-${UBUNTU_VERSION}-desktop-amd64.iso /mnt/
mkdir iso
