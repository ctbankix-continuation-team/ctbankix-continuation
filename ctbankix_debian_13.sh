...
#	Copyright (C) 2026 ctbankix-continuation-team
#
...
apt-get -y install build-essential fakeroot libncurses-dev xz-utils libssl-dev flex libelf-dev bison bc bison rsync debhelper libdw-dev
...
# Alten Kernel deinstallieren
chroot squashfs/ apt-get -y remove linux-image-*
...
chroot squashfs/ apt-get -y purge vim* oxygen-icon-theme xscreensaver* pocketsphinx-* qlipper fonts-urw-base35 gnome-accessibility-themes
...
chroot squashfs/ apt-get -y --no-install-recommends --no-install-suggests install squashfs-tools systemd-timesyncd bash-completion qpdfview screengrab package-update-indicator gnome-package-updater
chroot squashfs/ apt-get -y install network-manager nm-tray nm-tray-l10n wpasupplicant firmware-linux firmware-iwlwifi firmware-realtek
...
# Just-the-browser settings anwenden
FIREFOX_ETC_DIR=squashfs/etc/firefox-esr/policies/
mkdir -p $FIREFOX_ETC_DIR
wget -P $FIREFOX_ETC_DIR https://raw.githubusercontent.com/corbindavenport/just-the-browser/main/firefox/policies.json
...
Type=Application
Terminal=true
Icon=/usr/share/icons/Papirus/48x48/apps/kfloppy.svg
EOF
