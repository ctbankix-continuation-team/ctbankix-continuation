...
apt-get -y install git fakeroot libncurses-dev gawk flex bison openssl libssl-dev dkms libelf-dev libudev-dev libpci-dev libiberty-dev autoconf systemtap-sdt-dev libzstd-dev libbabeltrace-dev libperl-dev binutils-dev

# Quellcode des Kernels auschecken und in Verzeichnis wechseln
git clone --branch master --depth 1 git://kernel.ubuntu.com/ubuntu/ubuntu-focal.git kernel/current_head
...
sed -i "/^CONFIG_NVME_TCP / s/'-'/'n'/g" debian.master/config/annotations
# <<<<< Quellcode anpassen <<<<<
